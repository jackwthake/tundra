...
bool cull_chunk(chunk_t *chunk, void *param, usize num_params) {
  ...
  // Use same direction calculation as chunk loading
  float yaw = player->yaw;
  float cos_yaw = cosf(yaw);
  float sin_yaw = sinf(yaw);

  // Forward is -Z direction when yaw=0, right is +X
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    int world_dx = (int)roundf(rel_x * right_x + rel_z * forward_x);
    int world_dz = (int)roundf(rel_x * right_z + rel_z * forward_z);
...
void update_loaded_chunks(scene_t *scene) {
  ...
  if (++debug_frame % 60 == 0) {
    printf("Player at chunk (%d,%d), yaw=%.2f\n", player_chunk_x, player_chunk_z, scene->camera_pos.yaw);
  }


  // Get forward direction from camera yaw (forward is -Z)
  float yaw = scene->camera_pos.yaw;
  float cos_yaw = cosf(yaw);
  float sin_yaw = sinf(yaw);
  ...
  // Load chunks in priority order (forward chunks first)
  for (int i = 0; i < num_chunks; i++) {
    float rel_x = chunk_offsets[i][0];
    float rel_z = chunk_offsets[i][1];

    // Transform to world coordinates based on facing direction
    // Apply rotation matrix: [right_x forward_x][rel_x] = [world_dx]
    //                        [right_z forward_z][rel_z]   [world_dz]
    int world_dx = (int)roundf(rel_x * right_x + rel_z * forward_x);
    int world_dz = (int)roundf(rel_x * right_z + rel_z * forward_z);

    int chunk_x = player_chunk_x + world_dx;
    int chunk_z = player_chunk_z + world_dz;
    ...
    if (!is_chunk_loaded(&scene->chunk_map, chunk_x, chunk_z)) {
      ...
      if (debug_frame % 60 == 0) {
        printf("  Loading chunk (%d,%d) from rel(%.0f,%.0f)\n", chunk_x, chunk_z, rel_x, rel_z);
      }
